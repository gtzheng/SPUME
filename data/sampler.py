    ...
    def __iter__(self):
        ...
        for n in range(self.num_batches):
            ...
            for b in range(self.num_episodes):
                p_arr = np.random.choice(
                    self.confounders, len(self.classes), replace=False
                )
                ...
                for i in range(len(p_arr)):
                    group_cls_indexes.append(
                        [
                            group_indexes[i][group_cls_labels[i] == c]
                ...
                for i in range(len(p_arr)):
                    ...
                    if num_s < self.num_supp:
                        raise ValueError(
                            f"too large num_supp: Actual {num_s}; Required: {self.num_supp}"
                        )
                    indexes = np.random.permutation(len(group_cls_indexes[i][i]))[
                        0 : self.num_supp
                    ]
                ...
                for i in range(len(p_arr)):
                    ...
                    if num_q < self.num_query:
                        raise ValueError(
                            f"too large num_query: Actual {num_q}; Required: {self.num_query}"
                        )
                    indexes = np.random.permutation(len(group_cls_indexes[c][i]))[
                        0 : self.num_query
                    ]
        ...
        for n in range(self.num_batches):
            ...
            for b in range(self.task_num):
                ...
                for c in self.classes:
                    ...
                    prob_pos = (
                        self.class_correlated_feas[c][0]
                        - self.class_correlated_feas[c][0].min()
                        + 1e-10
                    )
                    prob_pos = prob_pos / prob_pos.sum()

                    while True:
                        attr_support_indexes = np.random.choice(
                            len(self.class_correlated_feas[c][1]),
                            2,
                            p=prob_pos,
                            replace=False,
                        )
                        ...
                        supp_all_indexes = indexes[
                            self.embeddings[self.y_array == c][:, attr_support].sum(
                                axis=1
                            )
                            > 0
                        ]
                        query_all_indexes = indexes[
                            self.embeddings[self.y_array == c][:, attr_query].sum(
                                axis=1
                            )
                            > 0
                        ]
                        supp_uni_indexes = list(
                            set(supp_all_indexes) - set(query_all_indexes)
                        )
                        query_uni_indexes = list(
                            set(query_all_indexes) - set(supp_all_indexes)
                        )
                        if len(supp_uni_indexes) > 0 and len(query_uni_indexes) > 0:
                            break

                    supp_sel_index = np.random.choice(
                        supp_uni_indexes, self.num_supp, replace=True
                    )
                    query_sel_index = np.random.choice(
                        query_uni_indexes, self.num_query, replace=True
                    )
...
class RandomTaskSampler:
    def __init__(
        self, dataset, num_supp, num_query, num_batches, task_num, save_path=None
    ):
        ...
        for n in range(self.num_batches):
            ...
            for b in range(self.task_num):
                ...
                for l in self.classes:
                    ...
                    idxes = np.random.permutation(len(sel_indexes))[
                        0 : self.num_supp + self.num_query
                    ]
